...
# s3 buckets
target_bucket_name = 'redfin-transform-zone-yml5'
...
def transform_data(task_instance):
    ...
    df["period_begin_in_months"] = df["period_begin_in_months"].replace(month_dict)
    df["period_end_in_months"] = df["period_end_in_months"].replace(month_dict)
...
with DAG('redfin_analytic_dag',
    ...
    load_to_s3 = BashOperator(
        ...
        bash_command='aws s3 mv {{ ti.xcom_pull("tsk_extract_redfin_data")[0] }} s3://store-raw-data-yml5',
    )
